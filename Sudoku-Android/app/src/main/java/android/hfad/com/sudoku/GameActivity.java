...
public class GameActivity extends AppCompatActivity {
    ...
    private void onClickTutorial() {
        ...
        dialog.setMessage(message).setTitle("Guide").show();
    }
    ...
    public void onClickSubmit(View view) {
        if (!grid.isLegalGrid()) {
            Toast.makeText(this, "Let's complete the table", LENGTH_LONG).show();
            return;
        }
        if (solver.checkValidGrid(grid.getNumbers())) {
            Toast.makeText(this, "Congratulations, correct answer", LENGTH_LONG).show();
            status = 0;
            if (status >= 0) {
                AlertDialog.Builder dialog = new AlertDialog.Builder(this, R.style.Theme_AppCompat_Light_Dialog);
                dialog.setMessage("Do you want to save the results on the leaderboard?")
                        .setTitle("Notify\n")
                        .setPositiveButton("Agree", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int i) {
